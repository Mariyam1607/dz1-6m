...
ДО РЕФАКТОРИНГА:
Все запросы, UI и бизнес-логика находились внутри компонентов.

ПОСЛЕ РЕФАКТОРИНГА:
Проект был разделен по слоям. Стало проще добавлять новый функционал, архитектура стала более масштабируемой.